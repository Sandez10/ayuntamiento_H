...
$actividad = isset($_POST['id_actividades']) ? intval($_POST['id_actividades']) : null;
    ...
    // Validación para el usuario "default"
    if (!$esAdmin) {
        // Verificar si el mes seleccionado es el mes anterior o el mes actual dentro de los primeros 7 días
        if (!($mesSeleccionado === $mesAnterior || ($mesSeleccionado === $mesActual && $diaActual <= 10))) {
            echo "<script>alert('Solo se puede registrar avance para el mes anterior o el mes actual (hasta el día 10).');</script>";
            exit(); // Detener la ejecución del script
        }
    }

    // Si es admin o la validación anterior pasó, proceder a insertar el avance
...
$queryActividades = "SELECT id_actividades, nombreActividad FROM listaactividades WHERE claveProgramaP = ? AND nombre_area = ?";// colocar parametro de nivel indicador = "Actividad"
...
<head>
    <meta charset="UTF-8">
    <title>SIPSEPP Registrar Avance</title>
...
<body>
    <div class="content">
        <header>
            <button type="button" class="btn-salir" onclick="location.href='../../sesiones_conexiones/destruir_sesion.php';">
            ...
            <form method="POST" enctype="multipart/form-data" action="" class="form-avance">
                ...
                <div class="container">
                    ...
                    <button id="toggleBtn" type="button">
                        <i class="fas fa-eye"></i> Mostrar Datos Generales
                    </button>
                ...
                <div class="row-group2">
                    ...
                    <?php
                        ...
                        for ($mes = 1; $mes <= 12; $mes++) {
                            ...
                            // Para usuarios no admin: Solo pueden seleccionar el mes anterior o el mes actual (si aún están dentro del límite del día 7)
                            $habilitadoParaUsuario = ($mes === $mesAnterior || ($mes === $mesActual && $diaActual <= 7));

                            // Si no es admin y el mes no está permitido, se deshabilita
                            $disabled = (!$esAdmin && !$habilitadoParaUsuario) ? 'disabled' : '';

                            // Marcar como seleccionado el mes actual
                    ...
                    <input type="number" name="avance" id="avance" placeholder="Ingresa el avance">

                    <label for="beneficiarios">Beneficiarios:</label>
                    <input type="number" name="beneficiarios" id="beneficiarios" required placeholder="Ingresa la cantidad de los beneficiarios">

                    <label for="evidencia">Evidencia en PDF:</label>
        ...
        <button type="button" class="btn btn-secondary" onclick="location.href='../../formularios/pre_a.php?claveProgramaP=<?= urlencode($claveProgramaP); ?>';">
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    toggleBtn.addEventListener("click", () => {
        ...
        if (datosGenerales.style.display === "none") {
            ...
            toggleBtn.innerHTML = '<i class="fas fa-eye-slash"></i> Ocultar Datos Generales'; // Cambiar texto y agregar ícono al botón
        } else {
            datosGenerales.style.display = "none";
            toggleBtn.innerHTML = '<i class="fas fa-eye"></i> Mostrar Datos Generales'; // Cambiar texto y agregar ícono al botón
